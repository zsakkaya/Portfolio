...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    ...
    emailjs.send(import.meta.env.VITE_EMAILJS_SERVICE_ID, import.meta.env.VITE_EMAILJS_TEMPLATE_ID,
      {
        from_name: form.name,
        to_name: 'Zeynep Serra',
        from_email: form.email,
        to_email: 'zsakkaya1@gmail.com',
      ...
      <motion.div className='flex-[0.75] bg-black-100 p-8 rounded-2xl' variants={slideIn('left', "tween", 0.2, 1)}>
        ...
        <form ref={formRef} onSubmit={handleSubmit} className='mt-12 flex flex-col gap-8'>
          ...
          <label className='flex flex-col'>
            ...
            <input type="email" name='email' value={form.email} onChange={handleChange} placeholder="What's your email address?" className='bg-tertiary py-4 px-6 placeholder:text-secondary text-white rounded-lg outlined-none border-none font-medium' />
          </label>
          <label className='flex flex-col'>
            <span className='text-white font-medium mb-4'>Your Message</span>
            <textarea rows="7" type="text" name='message' value={form.message} onChange={handleChange} placeholder="Write your message here" className='bg-tertiary py-4 px-6 placeholder:text-secondary text-white rounded-lg outlined-none border-none font-medium' />
          </label>
